...
QCoro::Task<QNetworkReply *> APIClient::slectProxies(const QString &proxy, const QString &name)
{
    QJsonObject body;
    body["name"] = name;
    return sendJsonData("PUT", QString("/proxies/%1").arg(proxy), body);
}

QCoro::Task<QNetworkReply *> APIClient::changeProxyMode(const QString &mode)
{
    QJsonObject body;
    body["mode"] = mode;
    return sendJsonData("PATCH", "/configs", body);
}

QCoro::Task<QNetworkReply *> APIClient::sendJsonData(const QString &method,
                                                     const QString &path,
                                                     const QJsonObject &body) const
{
    ...
    return Components::HttpClient::instance().awaitResquest(method, url, body, header);
}

QCoro::Task<QNetworkReply *> APIClient::configs()
{
    return get("/configs");
}
