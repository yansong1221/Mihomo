#include "SysProxyConfigurator.hpp"
...
bool __QueryProxyOptions()
{
    ...
    if (!InternetQueryOption(nullptr, INTERNET_OPTION_PER_CONNECTION_OPTION, &List, &nSize)) {
        qInfo() << "InternetQueryOption failed, GLE=" << QSTRN(GetLastError());
    }

    qInfo() << "System default proxy info:";

    if (Option[0].Value.pszValue != nullptr) {
#ifdef UNICODE
        qInfo() << QString::fromStdWString(Option[0].Value.pszValue);
#else
        qInfo() << QString::fromStdString(Option[0].Value.pszValue);
#endif
    }

    if ((Option[2].Value.dwValue & PROXY_TYPE_AUTO_PROXY_URL) == PROXY_TYPE_AUTO_PROXY_URL) {
        qInfo() << "PROXY_TYPE_AUTO_PROXY_URL";
    }

    if ((Option[2].Value.dwValue & PROXY_TYPE_AUTO_DETECT) == PROXY_TYPE_AUTO_DETECT) {
        qInfo() << "PROXY_TYPE_AUTO_DETECT";
    }

    if ((Option[2].Value.dwValue & PROXY_TYPE_DIRECT) == PROXY_TYPE_DIRECT) {
        qInfo() << "PROXY_TYPE_DIRECT";
    }

    if ((Option[2].Value.dwValue & PROXY_TYPE_PROXY) == PROXY_TYPE_PROXY) {
        qInfo() << "PROXY_TYPE_PROXY";
    }
...
#ifdef UNICODE
        qInfo() << QString::fromStdWString(Option[4].Value.pszValue);
#else
        qInfo() << QString::fromStdString(Option[4].Value.pszValue);
#endif
    }
...
bool __SetProxyOptions(LPTSTR proxy_full_addr, bool isPAC)
{
    INTERNET_PER_CONN_OPTION_LIST list;
    BOOL bReturn;
    ...
    if (isPAC) {
        qInfo() << "Setting system proxy for PAC";
        //
        ...
        qInfo() << "Setting system proxy for Global Proxy";
        //
        ...
        auto localhost = TEXT("localhost");
        list.pOptions[2].Value.pszValue = const_cast<LPWSTR>(localhost);
    }
...
void SysProxyConfigurator::SetSystemProxy(const QString &address, int httpPort, int socksPort)
{
    ...
    qInfo() << "Windows proxy string: " + __a;
    auto proxyStrW = new TCHAR[__a.length() + 1];
#ifdef UNICODE
    lstrcpy(proxyStrW, __a.toStdWString().c_str());
#else
    lstrcpy(proxyStrW, __a.toStdString().c_str());
#endif
    //
    __QueryProxyOptions();

    if (!__SetProxyOptions(proxyStrW, false)) {
        qInfo() << "Failed to set proxy.";
    }
        ...
        for (const auto &protocol : QStringList{"http", "ftp", "https"}) {
            // for GNOME:
            {
                actions << ProcessArgument{"gsettings",
                                           {"set",
                                            "org.gnome.system.proxy." + protocol,
                                            "host",
                                            address}};
                actions << ProcessArgument{"gsettings",
                                           {"set",
    ...
    for (const auto &action : actions) {
        ...
        qDebug() << QString("[%1] Program: %2, Args: %3")
                        .arg(returnCode)
                        .arg(action.first)
                        .arg(action.second.join(";"));
        // give the code back
        results << (returnCode == QProcess::NormalExit);
    }

    if (results.count(true) != actions.size()) {
        qDebug() << "Something wrong when setting proxies.";
    }
...
void SysProxyConfigurator::ClearSystemProxy()
{
    qInfo() << "Clearing System Proxy";

#ifdef Q_OS_WIN
    qInfo() << "Cleaning system proxy settings.";
    INTERNET_PER_CONN_OPTION_LIST list;
    BOOL bReturn;
    DWORD dwBufSize = sizeof(list);
    ...
    if (nullptr == list.pOptions) {
        // Return FALSE if the memory wasn't allocated.
        qInfo() << "Failed to allocat memory in DisableConnectionProxy()";
    }
